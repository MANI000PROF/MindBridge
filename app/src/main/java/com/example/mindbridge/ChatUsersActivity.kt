...
import com.example.mindbridge.databinding.ActivityChatUsersBinding
...
class ChatUsersActivity : AppCompatActivity() {
    ...
    private var binding: ActivityChatUsersBinding? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dialog = ProgressDialog(this@ChatUsersActivity)
        ...
        userAdapter = UserAdapter(this@ChatUsersActivity, users, object : UserAdapter.OnItemClickListener {
            override fun onItemClick(user: User) {
                // Start ChatActivity
                Log.d("UserAdapter", "Clicked on: ${user.name}")
                val intent = Intent(this@ChatUsersActivity, chatActivity::class.java).apply {
        ...
        binding!!.mRec.layoutManager = GridLayoutManager(this@ChatUsersActivity, 2)
        ...
        database!!.reference.child("users").addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                Log.d("ChatUsersActivity", "Users count: ${users.size}")
                userAdapter.notifyDataSetChanged() // Refresh the adapter
            }

            override fun onCancelled(error: DatabaseError) {
                Log.e("ChatUsersActivity", "Database error: ${error.message}")
            }
        })
            ...
            database!!.reference.child("users").child(currentUserId).addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    currentUser?.let {
                        ...
                        Glide.with(this@ChatUsersActivity)
                ...
                override fun onCancelled(error: DatabaseError) {
                    Log.e("ChatUsersActivity", "Error loading user details: ${error.message}")
                }
            })
        }
    }
    ...
    private fun populateMentorsFromUsers() {
        database!!.reference.child("users").addListenerForSingleValueEvent(object : ValueEventListener {
            ...
            override fun onCancelled(error: DatabaseError) {
                Log.e("ChatUsersActivity", "Error populating mentors: ${error.message}")
            }
        })
    }
    ...
    override fun onStart() {
        ...
        populateMentorsFromUsers() // Populate mentors whenever ChatUsersActivity starts
    }
        ...
        user.uid?.let {
            database!!.reference.child("users").child(it).removeValue()
                .addOnSuccessListener {
                    Log.d("ChatUsersActivity", "${user.name} deleted successfully")
                    users.remove(user) // Remove from local list
                    userAdapter.notifyDataSetChanged() // Notify adapter
                }
                .addOnFailureListener { error ->
                    Log.e("ChatUsersActivity", "Failed to delete user: ${error.message}")
                }
        }
    }
    ...
    override fun onResume() {
        ...
        if (currentId != null) {
            ...
            val intent = Intent(this@ChatUsersActivity, OTPActivity::class.java)
            intent.flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK
            startActivity(intent)
            finish() // Finish the ChatUsersActivity
        }
    }
    ...
    private fun showLogoutDialog() {
        ...
        builder.setPositiveButton("Yes") { dialog: DialogInterface, which: Int ->
            ...
            val intent = Intent(this@ChatUsersActivity, Login::class.java) // Redirect to OTPActivity
